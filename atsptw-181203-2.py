...
if klastrun <> 0 :
	...
	for krun in range(abs(klastrun)+1):
		...
		if os.path.isfile(kfilesol) : 
			...
			for myrow in mybuf:
				wvariable = myrow[0]
				wvalue	  = float(myrow[1])
				if wvariable[0:4] == "pend" and wvalue <= 0.01 :	
	...
	if runopt and worstrun <> -99 and not xhardend :
		if 	objopt == 1 :
			...
			print("*** Looking (1) to optimize order: ",xorderdel, " from route # ", xroutedel)	
			klastrun = klastrun - 1 
	elif klastrun >= 1 :
		##########################################################################################################################
		#### MATRICS PART
...
while True:
	...
	xcom     = "solverx.exe " + wfile + " " + kfilesol + " quit"
	
	# solverx.exe myprob-6.mps sol-6.sol quit
	if runopt :
		if os.path.isfile(kfilesol) :
			os.remove(kfilesol)
	...
	if os.path.isfile(kfilesol) : 
		...
		if not runopt :
			...
			xcount = 20
			xrouteinc = nrun
			for krun in range(xrouteinc,klastrun+1):  
				if numpy.sum(xsol[krun][xdep[worstord]]) >= 1 :  
					xsum = numpy.sum(xsol[krun]) 
...
if wantopt :
	if not runopt :  
		...
		if objopt == 2 :
			...
			if xcount <= 4 :
				xorderdel = 76
				print("Ruote to try to eliminate:", xroutedel)
				print("Order to try to eliminate:", xorderdel)
